...
```bash
npx skills add vagabondms/skills --skill=grill-me
```
...
[Source](https://github.com/vagabondms/skills/tree/main/skills/productivity/grill-me)
...
## The plan review (fork customization)

In this fork, a `grill-me` session doesn't end at shared understanding — it ends by firing [grill-to-plan](https://aihero.dev/skills-grill-to-plan), which writes the settled plan and opens it in your browser as an HTML **plan review**: each touched module's interface rendered as a git-style diff, plus build order and open questions, gated on your approval. The session announces this deliverable up front, so the plan never gets lost in a long interview. Signatures in the plan are contracts: implementation fills in bodies, and any signature change gets flagged back first.
