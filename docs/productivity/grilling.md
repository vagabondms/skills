...
```bash
npx skills add vagabondms/skills --skill=grilling
```
...
[Source](https://github.com/vagabondms/skills/tree/main/skills/productivity/grilling)
...
It asks **one question at a time** and waits for your answer before the next — never a bulk list, which is bewildering. Every question arrives **context-first**: before asking, the agent states what prompted the question, where it sits in the decision tree, and what hinges on the answer, so you're never answering cold. Each question also comes with the agent's own recommended answer, and any question the codebase can settle it explores instead of asking you. It won't start enacting the plan until you confirm the shared understanding has been reached.
